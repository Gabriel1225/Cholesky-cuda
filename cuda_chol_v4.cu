...
__global__ void chol_kernel(double * U)
{
	...
	//__shared__ double temp[MATRIX_SIZE*MATRIX_SIZE];
	__shared__ double temp[MATRIX_SIZE*MATRIX_SIZE];

	for(i=bid*THREAD_NUM+tid;i<num_rows;i+=BLOCK_NUM*THREAD_NUM)
	{
		for (j=0;j<num_rows;j++)
		{
			//temp [i*num_rows+j] = 0.0;
			temp[i*num_rows+j] = U[i*num_rows+j];
			U[i*num_rows+j] = 0.0;
		}
	}
	__syncthreads();
	...
	for (k =0;k<num_rows;k++)
	{
		if (tx == 0)
		{
			temp[k*num_rows+k] = sqrt(temp[k*num_rows+k]);
			for (j=(k+1);j<num_rows;j++)
			{
				temp[k * num_rows + j] /=  temp[k * num_rows + k];
			}
		}
		__syncthreads();
		...
		for (i=(k+1)+bid*THREAD_NUM+tid;i<num_rows;i+=BLOCK_NUM*THREAD_NUM)
		{	
			for (j=i;j<num_rows;j++)
				{
					temp[i*num_rows+j] -= temp[k*num_rows+i]*temp[k*num_rows+j];
				}
		}
		__syncthreads();
	}
	...
	// 		U[i*num_rows+j] = 0.0;
	// 		if(i<j)
	// 		U[i*num_rows+j] = temp[i+num_rows+j];
	// 	}
	// }
	for(j=bid*THREAD_NUM+tid;j<num_rows;j+=BLOCK_NUM*THREAD_NUM)
	{
		for(i=0;i<=j;i++)
		{
			U[i*num_rows+j] = temp[i*num_rows+j];
...
int main()
{
	...
	int num_block = BLOCK_NUM;
	int threads_per_block = THREAD_NUM;
	//float ops_per_thread = MATRIX_SIZE / (num_block*threads_per_block);
